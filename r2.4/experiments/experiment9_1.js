...
window.initExperiment = function() {
  ...
  const sketch = (p) => {
    ...
    //----------------------------------------------------
    p.draw = () => {
      // 1) Set background based on dark mode
      p.background(darkMode ? 0 : 255);

      // 2) Draw the grid with colors based on colorMap
      ...
      // 4) Draw paddles based on dark mode
      p.stroke(darkMode ? 255 : 0);
      p.fill(darkMode ? 255 : 200); // White paddles in dark mode, light gray in light mode
      // Left paddle
      ...
      // 6) Draw the ball with its current color
      let halfSize = (cellW * BALL_SIZE_FACTOR) / 2;
      let ballRGB = getMixedColorFromSet(new Set([ballColor]));
      p.fill(ballRGB.r, ballRGB.g, ballRGB.b);
      p.noStroke();
    ...
    function updateBall() {
      ...
      // Left/Right reset instead of bounce
      let halfSize = (cellW * BALL_SIZE_FACTOR) / 2;
      if (ballX < halfSize) {
        // If the ball touches the left edge, reset
        resetBall();
        cycleBallColor(); // optional color cycle on reset
      } else if (ballX > p.width - halfSize) {
        // If the ball touches the right edge, reset
        resetBall();
        cycleBallColor(); // optional color cycle on reset
      }
    ...
    function getMixedColorFromSet(colorSet) {
      if (colorSet.size === 0) {
        // no color => white for light mode, black for dark mode
      ...
      let hex = colorMap[sorted] || (darkMode ? "#FFFFFF" : "#000000");
      return hexToRGB(hex);
    }
